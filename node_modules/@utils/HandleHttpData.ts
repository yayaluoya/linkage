...
export class HandleHttpData {
     ...
     * 处理数据
     */
    static handle<T = any>(_data: T, _handType: ComN.dataHandlesType[] = [], C: Crypto): T {
        //克隆一个新的数组，防止污染
        _handType = [..._handType];
        try {
            //遍历处理类型
            for (let o of _handType) {
                switch (o) {
                    case 'e':
                        _data = C.encryptionData(JSON.stringify(_data)) as any;
                        break;
                    case 'z':
     ...
     * 解析数据
     */
    static handle_<T = any>(_data: T, _handType: ComN.dataHandlesType[] = [], C: Crypto): T {
        //克隆一个新的数组，防止污染
        _handType = [..._handType];
        try {
            for (let o of _handType.reverse()) {
                switch (o) {
                    case 'e':
                        _data = JSON.parse(C.decryptionData(_data as any));
                        break;
                    case 'z':
